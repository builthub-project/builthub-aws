...
## @type: DataSource
## @args: [database = "builthubgluedb", table_name = "builthub_dataset005", transformation_ctx = "DataSource0"]
## @return: DataSource0
## @inputs: []
DataSource0 = glueContext.create_dynamic_frame.from_catalog(database = "builthubgluedb", table_name = "builthub_dataset005", transformation_ctx = "DataSource0")
## @type: ApplyMapping
...
Transform0 = ApplyMapping.apply(
    frame = DataSource0, mappings = [
        ...
        ("mesurementunit", "string", "mesurementunit", "string"),
        ("sector", "string", "sector", "string"),
        ("value", "string", "value", "float")
    ], 
...
dsOwnerID = URIRef(DEFAULT_NS+r"Dataset/05") # Dataset which owns these entities
baseEntityName = "Dataset005/"
...
def processPartitionGraph(rowset):
    ...
    for row in rowset:
        ...
        graph.add((rootNode, RDF.type , BUILTHUB.Dataset005))  #CHANGE NAME IF NEEDED
    ...
    s3.put_object(Body=turtleContent, Bucket="builthub-inbox-dev", Key="neptune/inbox/dataset005-" + uuid.uuid4().hex + ".ttl", ContentEncoding="UTF-8", ContentType="text/turtle")
    #s3.put_object(Body=turtleContent, Bucket="builthubaws", Key="neptune/inbox/dataset005-" + uuid.uuid4().hex + ".ttl", ContentEncoding="UTF-8", ContentType="text/turtle")
