...
## @type: DataSource
## @args: [database = "builthubgluedb", table_name = "builthub_dataset014", transformation_ctx = "DataSource0"]
## @return: DataSource0
## @inputs: []
DataSource0 = glueContext.create_dynamic_frame.from_catalog(database = "builthubgluedb", table_name = "builthub_dataset014", transformation_ctx = "DataSource0")
## @type: ApplyMapping
## @args: [mappings = [("country", "string", "country", "string"), ("year", "string", "year", "string"), ("total_energy_consumed", "string", "total_energy_consumed", "float")], transformation_ctx = "Transform0"]
## @return: Transform0
## @inputs: [frame = DataSource0]
Transform0 = ApplyMapping.apply(frame = DataSource0, mappings = [("country", "string", "country", "string"), ("year", "string", "year", "string"), ("total_energy_consumed", "string", "total_energy_consumed", "float")], transformation_ctx = "Transform0")
## @type: DataSink
...
dsOwnerID = URIRef(DEFAULT_NS+r"Dataset/14") # Dataset which owns these entities
baseEntityName = "Dataset014/"
...
def processPartitionGraph(rowset):
    ...
    for row in rowset:
        ...
        graph.add((rootNode, RDF.type , BUILTHUB.Dataset014))  #CHANGE NAME IF NEEDED
    ...
    s3.put_object(Body=turtleContent, Bucket="builthub-inbox-dev", Key="neptune/inbox/dataset014-" + uuid.uuid4().hex + ".ttl", ContentEncoding="UTF-8", ContentType="text/turtle")
    #s3.put_object(Body=turtleContent, Bucket="builthubaws", Key="neptune/inbox/dataset014-" + uuid.uuid4().hex + ".ttl", ContentEncoding="UTF-8", ContentType="text/turtle")
