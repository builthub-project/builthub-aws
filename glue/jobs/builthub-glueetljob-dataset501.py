...
## @type: DataSource
## @args: [database = "builthubgluedb", table_name = "builthub_dataset501", transformation_ctx = "DataSource0"]
## @return: DataSource0
## @inputs: []
DataSource0 = glueContext.create_dynamic_frame.from_catalog(database = "builthubgluedb", table_name = "builthub_dataset501", transformation_ctx = "DataSource0")
## @type: ApplyMapping
...
dsOwnerID = URIRef(DEFAULT_NS+r"Dataset/501") # Dataset which owns these entities
baseEntityName = "Dataset501/"
...
def processPartitionGraph(rowset):
    ...
    for row in rowset:
        ...
        graph.add((rootNode, RDF.type , BUILTHUB.Dataset501))  #CHANGE NAME IF NEEDED
        ...
        if(valueC=="Germany (until 1990 former territory of the FRG)"):
            valueC="Germany"
    ...
    s3.put_object(Body=turtleContent, Bucket="builthub-inbox-dev", Key="neptune/inbox/dataset501-" + uuid.uuid4().hex + ".ttl", ContentEncoding="UTF-8", ContentType="text/turtle")
    #s3.put_object(Body=turtleContent, Bucket="builthubaws", Key="neptune/inbox/dataset501-" + uuid.uuid4().hex + ".ttl", ContentEncoding="UTF-8", ContentType="text/turtle")
