...
SELECT DISTINCT ?identifier ?nutsID ?locationID ?sector ('5'^^xsd:integer as ?datasetID) ?startDate ?endDate ?msrValue ?measuredElement ?unit 
WHERE {
...
#DELETE Data
cursor.execute("DELETE FROM \"public\".\"dataset005\"")
connection.commit()

cursor.execute("DELETE FROM \"public\".\"dataset_period\" WHERE dataset_id = 5")
connection.commit()
...
for result in sparql.query().bindings:
    try:
        ...
        cursor.execute("INSERT INTO \"public\".\"dataset005\" (\"record_id\", \"nuts_code\", \"location_id\", \"dataset_id\", \"sector\", \"msrvalue\", \"measuredelement\", \"msrunit\") VALUES ('"+result["identifier"].value+"','"+result["nutsID"].value+"','"+result["locationID"].value+"','"+result["datasetID"].value+"','"+result["sector"].value+"',"+result["msrValue"].value+",'"+result["measuredElement"].value+"','"+result["unit"].value+"')")
        connection.commit()
    except:
