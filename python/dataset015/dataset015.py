...
SELECT DISTINCT ?identifier ?nutsID ?location_id ('15'^^xsd:integer as ?datasetID) ?siec ?freq ?startDate ?endDate ?msrValue ?measuredElement ?msrUnit WHERE {
...
#DELETE Data
cursor.execute("DELETE FROM \"public\".\"dataset015\"")
connection.commit()

cursor.execute("DELETE FROM \"public\".\"dataset_period\" WHERE dataset_id = 15")
connection.commit()
